...
Codex 会用 `npx skills add` 安装技能、安装 AGENTS 门控、展示完整的 GPT-5.6 角色表，并让你选择推荐组合、全部角色或自定义组合。选中的同名角色会被覆盖，随后 Codex 会提示你重启。
...
使用重复的 `--role` 参数可以选择其他组合。仅项目安装时，把 `--scope global` 换成 `--scope project --path /path/to/repo`。每个选中的角色都会覆盖已有同名角色，未选角色保持不变。没有捆绑角色时，cast-subagents 仍可工作，但首选阵容可能因角色缺失而降级。
...
|---:|---|---|---|---|
| 1 | `code-mapper` | `gpt-5.6-terra` | `medium` | 追踪代码路径、文件、符号和归属边界 |
...
| 8 | `test-engineer` | `gpt-5.6-luna` | `high` | 针对行为和风险设计最小测试覆盖 |
| 9 | `test-automator` | `gpt-5.6-terra` | `xhigh` | 在行为明确后添加有边界的回归测试 |
| 10 | `web-performance-auditor` | `gpt-5.6-luna` | `high` | 审计 Web 性能证据和 Core Web Vitals 风险 |
